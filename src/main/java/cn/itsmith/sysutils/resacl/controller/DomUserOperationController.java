...
public class DomUserOperationController {
    ...
    @ApiOperation(value = "资源授权的添加", notes = "针对资源授权的添加操作  \n" +
            ...
            "2.已经存在这条资源授权记录，不能重复添加【对应异常代码 3002】  \n" +
            "...")
    @RequestMapping(value="/user-operations",method = RequestMethod.POST)
    public ResultUtils addUserOperations(@RequestBody List<DomUserOperationA> domUserOperationAS, @RequestHeader(value = "token", required = true) String validate){
        List<DomUserOperation> domUserOperations = new ArrayList<>();
        for(DomUserOperationA domUserOperationA : domUserOperationAS){
            domainService.verify(domUserOperationA.getDomId(), validate);
            ...
            domUserOperation.setOwnerId(domUserOperationA.getOwnerId());
            domUserOperation.setUserOwnerId(domUserOperationA.getUserOwnerId());
            domUserOperations.add(domUserOperation);
        }

        return domUserOperationService.addOps(domUserOperations);
